...
export function GalleryPage() {
  ...
  const title = lang === 'ta' && project.title_ta ? project.title_ta : project.title;
  const description = lang === 'ta' && project.description_ta ? project.description_ta : project.description;
    ...
    <div className="min-h-screen">
      <div className="bg-wood-900 text-white py-16 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          <h1 className="font-serif text-4xl md:text-5xl font-semibold text-white">{t('gallery_title', lang)}</h1>
          <p className="text-wood-300 mt-3 text-lg">{t('gallery_subtitle', lang)}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-8">
          ...
          >
            {/* After (background) */}
            <img src={project.after_image} alt={t('gallery_after', lang)} className="absolute inset-0 w-full h-full object-cover" />
            {/* Before (clipped) */}
            <div className="absolute inset-0 overflow-hidden" style={{ width: `${sliderPos}%` }}>
              <img src={project.before_image} alt={t('gallery_before', lang)} className="absolute inset-0 h-full object-cover" style={{ width: containerRef.current?.clientWidth || '100%' }} />
            </div>
            ...
            {/* Slider line */}
            <div className="absolute top-0 bottom-0 w-1 bg-white shadow-lg pointer-events-none" style={{ left: `${sliderPos}%` }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="md:col-span-2">
            <h2 className="font-serif text-2xl font-semibold text-wood-900 mb-2">{title}</h2>
            <p className="text-wood-600 leading-relaxed">{description}</p>
          </div>
          <div className="card p-4">
            <div className="text-xs text-wood-400 mb-1">{lang === 'ta' ? 'இடம்' : 'Location'}</div>
            <p className="font-medium text-wood-800">{project.location}</p>
            <div className="text-xs text-wood-400 mt-3 mb-1">{lang === 'ta' ? 'அறை வகை' : 'Room Type'}</div>
          ...
          {projects.map((p, i) => (
            <button
              ...
              className={`group relative aspect-video rounded-xl overflow-hidden border-2 transition-all ${
                i === activeProject ? 'border-wood-700 scale-105' : 'border-transparent opacity-70 hover:opacity-100'
              }`}
            >
              <img src={p.after_image} alt="" className="w-full h-full object-cover" />
